...
面向网页版抖音的浏览器扩展。抖音网页端不会记住「屏蔽礼物特效 / 关闭送礼信息 / 最高画质 / 关闭 AI 入口」这类选择，这个扩展每次进入页面都会替你重新校验并补上。
...
7. **关闭头像上方【AI抖音】入口**  
   MAIN world 把 `aiEntryClose` 写成 `1`（抖音用 `1` 表示关闭），并用 CSS 隐藏右侧栏 `.ai-douyin-entry`。不碰左侧导航里的「AI抖音」。
8. **隐藏标题下方花片**  
   去掉汽水音乐、相关搜索、识别画面和「个人观点，仅供参考」。作者、时间、正文 `#标签` 和「合集」留下。
9. **隐藏「下载抖音精选」**  
   CSS 去掉左侧栏底部的客户端下载推广条，导航项不动。
10. **隐藏「听抖音」**  
    去掉右侧互动栏分享下方的听抖音入口，点赞 / 评论 / 收藏 / 分享 / 更多不动。
11. **隐藏「大家都在搜」**  
    评论侧栏顶部的相关搜索条去掉，「全部评论」还在。
